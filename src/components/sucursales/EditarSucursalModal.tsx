...
import { X, Pencil, ChevronDown, CheckCircle } from 'lucide-react'
import type { Sucursal, SucursalInput } from '../../types'

interface Props {
  sucursal: Sucursal
  onClose:  () => void
  onUpdate: (id: string, data: Partial<SucursalInput>) => Promise<void>
}
...
// ─── Modal ────────────────────────────────────────────────────────────────────
export default function EditarSucursalModal({ sucursal, onClose, onUpdate }: Props) {
  const [form, setForm] = useState<SucursalInput>({
    nombre:           sucursal.nombre,
    direccion:        sucursal.direccion,
    horario_apertura: sucursal.horario_apertura === '—' ? '' : sucursal.horario_apertura,
    horario_cierre:   sucursal.horario_cierre   === '—' ? '' : sucursal.horario_cierre,
    telefono:         sucursal.telefono ?? '',
    edge_device_id:   sucursal.edge_device_id ?? '',
  })
  const [is24h, setIs24h]     = useState(false)
  const [saving, setSaving]   = useState(false)
  const [error, setError]     = useState<string | null>(null)
  const prevTimes = useRef({ apertura: form.horario_apertura, cierre: form.horario_cierre })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      await onUpdate(sucursal.id, form)
      onClose()
    } catch (err: any) {
  ...
  return (
    <div
      data-testid="editar-sucursal-modal"
      style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{
              ...
              <Pencil size={15} color="var(--green)" strokeWidth={2} />
            </div>
            <div>
              <p style={{ fontSize: 14, fontWeight: 800, color: 'var(--text)' }}>Editar Sucursal</p>
              <p style={{ fontSize: 10, color: 'var(--muted)' }}>{sucursal.nombre}</p>
            </div>
          </div>
          <button
            data-testid="editar-sucursal-modal-close"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <input
              data-testid="editar-sucursal-input-nombre"
          ...
          <div>
            ...
            <input
              data-testid="editar-sucursal-input-direccion"
              value={form.direccion} onChange={set('direccion')} style={inputStyle}
            />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <TimeField
              ...
              disabled={is24h}
              testId="editar-sucursal-select-apertura"
            />
            <TimeField
              label="Cierre *"
              ...
              disabled={is24h}
              testId="editar-sucursal-select-cierre"
            />
          </div>
          ...
          <div>
            ...
            <input
              data-testid="editar-sucursal-input-telefono"
          ...
          <div>
            ...
            <input
              data-testid="editar-sucursal-input-edge"
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
            ...
            <button
              data-testid="editar-sucursal-btn-guardar"
              type="submit"
            ...
            >
              {saving ? (<><Spinner color="var(--muted)" /> Guardando…</>) : 'Guardar cambios'}
            </button>
          </div>
        </form>
      </div>
